#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# GL.iNet GL-E5800 (Mudi 7) — SSH-Manager
#
...
#   setup-modem        5G-Modem aktivieren (SIM eingesteckt?)
#
...
#   ip                 Alle Netzwerk-IPs anzeigen (inkl. 5G)
#
...
#   ./ssh-manager.sh -h 192.168.10.195 status
#   ./ssh-manager.sh setup-modem
...
_setup_key() {
    ...
    [ -f "$keyfile" ] || err "SSH-Public-Key nicht gefunden: $keyfile
     Erstellen: ssh-keygen -t ed25519 -f ~/.ssh/gl-e5800"
...
_setup_ip() {
    local wifi_ip="${1:-192.168.10.195}"
    local eth_ip="${2:-192.168.10.192}"
...
_setup_modem() {
    step "5G-Modem aktivieren (GL-E5800 SIM/eSIM)..."
    _ssh "
echo '--- Modem-Status ---'
ls /dev/cdc* /dev/qmi* /dev/wwan* 2>/dev/null || echo 'Kein Modem-Device gefunden (SIM eingesteckt?)'
echo ''
echo '--- 5G aktivieren ---'
...
uci set network.modem.disabled='0'
uci commit network
/etc/init.d/network restart
sleep 5
echo '--- Modem-IP ---'
ip addr show wwan0 2>/dev/null | grep 'inet ' || echo 'wwan0 noch keine IP (Modem startet...)'
"
    ok "5G-Modem aktiviert. Prüfe mit: ./ssh-manager.sh -h $SSH_HOST ip"
}
...
_setup_all() {
    echo ""
    echo -e "${B}${C}╔══════════════════════════════════════════════════════╗"
    echo -e "║  GL.iNet GL-E5800 (Mudi 7) — Ersteinrichtung        ║"
    echo -e "╚══════════════════════════════════════════════════════╝${N}"
    echo ""
    ...
    printf "  WiFi-IP (Standard: 192.168.10.195): "; read -r wip; wip="${wip:-192.168.10.195}"
    printf "  Ethernet-IP (Standard: 192.168.10.192): "; read -r eip; eip="${eip:-192.168.10.192}"
    _setup_ip "$wip" "$eip"

    step "Phase 7 — 5G-Modem (optional)"
    printf "  5G-Modem aktivieren? [j/N] "; read -r modem_ans
    if [ "$modem_ans" = "j" ] || [ "$modem_ans" = "J" ]; then
        SSH_HOST="$wip"
        _setup_modem
    fi

    step "Phase 8 — Verifikation"
    sleep 3
    SSH_HOST="$wip"
    _cmd_status
    ...
    echo ""
    echo -e "${B}${G}╔══════════════════════════════════════════════════════╗"
    echo -e "║  ✓ GL-E5800 Einrichtung abgeschlossen!               ║"
    echo -e "╚══════════════════════════════════════════════════════╝${N}"
    echo ""
...
if [ -n \"\$BID\" ]; then
    usbip bind -b \"\$BID\" && echo \"Adapter gebunden: \$BID ✓\" || echo 'Bind fehlgeschlagen (evt. schon gebunden)'
else
...
echo ''
echo '5G-Modem:'
ip addr show wwan0 2>/dev/null | grep -E 'inet |^[0-9]+:' || echo '  wwan0 nicht aktiv (setup-modem ausführen?)'
echo ''
echo 'Routen:'
...
_usage() {
    echo ""
    echo -e "${B}GL.iNet GL-E5800 (Mudi 7) — SSH-Manager${N}"
    echo ""
    echo -e "${B}VERWENDUNG:${N}  ./ssh-manager.sh [OPTIONEN] BEFEHL [ARGUMENTE]"
    echo ""
    echo "  setup               Vollständige Ersteinrichtung (8 Phasen)"
    echo "  setup-modem         5G-Modem aktivieren"
    echo "  setup-obd           OBD2-Service einrichten"
    echo "  setup-display       Display-Service einrichten"
    echo "  setup-wifi SSID PW  WiFi-Client konfigurieren"
    echo "  setup-ip [WIP] [EIP] Statische IPs (Standard: .195/.192)"
...
case "$COMMAND" in
    ...
    setup-modem)     _check_reachable; _setup_modem ;;
