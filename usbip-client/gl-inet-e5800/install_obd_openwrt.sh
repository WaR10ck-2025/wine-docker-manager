#!/bin/sh
# ─────────────────────────────────────────────────────────────────────────────
# OBD2 Monitor — OpenWrt Setup-Skript für GL.iNet GL-E5800 (Mudi 7)
#
# Unterschied zu GL-BE3600: 8 GB eMMC → Python-venv auf Flash (kein USB-Stick!)
#
# Auf dem GL-E5800 als root ausführen:
...
info "OBD2 Monitor Setup — GL.iNet GL-E5800 (Mudi 7 — 8 GB eMMC)"
echo ""

# ── 1. Speicher prüfen (eMMC, kein USB-Stick nötig) ──────────────────────────
...
if [ "$FREE_MB" -lt 200 ]; then
    ...
    warning "Falls zu wenig: pip --target /mnt/usb/obd-pkgs als Fallback (USB-Stick)"
else
...
if [ -f "$VENV_DIR/bin/activate" ]; then
    ...
    python3 -m venv "$VENV_DIR" 2>/dev/null || {
        # Fallback: venv-Modul installieren
...
# OBD2 Monitor Service — GL.iNet GL-E5800 (Mudi 7)
...
info ""
info "GL-E5800 LAN-IP:     ${LAN_IP}"
[ -n "$WIFI_IP" ] && info "GL-E5800 WiFi-IP:    ${WIFI_IP}"
info "OBD2 Port:           8765"
